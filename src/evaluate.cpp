  ...
  Score Evaluation<T>::pieces() {
    ...
    while ((s = *pl++) != SQ_NONE)
    {
        ...
        if (Pt == BISHOP || Pt == KNIGHT)
        {
            ...
            if (bb & s)
                score += Outpost[Pt == BISHOP][bool(attackedBy[Us][PAWN] & s)] * (Pt == BISHOP ? 2 : pe->semiopen_file(Us, file_of(s)) ? 5 : 4);

            else if (bb &= b & ~pos.pieces(Us))
                score += Outpost[Pt == BISHOP][bool(attackedBy[Us][PAWN] & bb)] * (Pt == BISHOP ? 1 : pe->semiopen_file(Us, file_of(s)) ? 3 : 2);
