  ...
  Score Evaluation<T>::pieces() {
    ...
    while ((s = *pl++) != SQ_NONE)
    {
        ...
        if (Pt == BISHOP || Pt == KNIGHT)
        {
            ...
            if (bb & s)
                score += Outpost[Pt == BISHOP][bool(attackedBy[Us][PAWN] & s)] * (Pt == BISHOP ? 2 : (!pos.can_castle(Them) && (file_of(pos.square<KING>(Them)) - file_of(s) > 4) ? 3 : 4));

            else if (bb &= b & ~pos.pieces(Us))
                score += Outpost[Pt == BISHOP][bool(attackedBy[Us][PAWN] & bb)] * (Pt == BISHOP ? 1 : (!pos.can_castle(Them) && (file_of(pos.square<KING>(Them)) - file_of(s) > 4) ? 1 : 2));
